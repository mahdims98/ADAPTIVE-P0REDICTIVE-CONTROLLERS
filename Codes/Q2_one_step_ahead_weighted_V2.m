...
sysD = z^-(delay) * (z-0.75) / ((z-0.25)*(z-0.85));
rlocus(z^-(delay) * 1/sysD)  %%%%%%% attention needed (delay-1)
...
main_folder_name = "one-step-ahead-weighted";
...
for i = skip_instances:num_samples-d     
    phi_t = [[-y(i-1:-1:max(i-(length(A) - 1), 1)), zeros(1, 1-(i-(length(A) - 1)))], [u(i-delay:-1:max(i-length(B_prime)-delay+1, 1)), zeros(1, 1-(i-length(B_prime)-delay+1))]].'; 
    ...
    u(i) = beta_0 * (y_star(i+d) - filter_signal(alpha, y, i) - filter_signal(beta_prime, u, i-1))/(beta_0^2 + landa);
